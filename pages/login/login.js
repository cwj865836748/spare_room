...
Page({
  ...
  getPhoneNumber(e) {
    ...
    request({
      ...
      result.data.nickname ? wx.navigateBack({
        delta: 1
      }) : wx.redirectTo({
  ...
  getCode() {
    ...
    if (!isNull({
        content: !checkPhone(this.data.phone),
        title: '手机号码格式',
        topic: '有误'
      })) {
      return
  ...
  codeLogin() {
    ...
    isNull({
      content: !checkPhone(phone),
      title: '手机号码格式',
      topic: '有误'
    }) && request({
      url: api.authorization.codeLogin,
      data: {
        phone,
        phone_code,
        code,
