...
Page({
  ...
  getHotelList() {
    ...
    request({
      url: api.index.hotelList,
      data,
      header
    }).then(res => {
      this.setData({
        ['hotelList[' + this.data.listIndex + ']']: res.data.list,
  ...
  showSort(e) {
    ...
    this.setData({
      ...
      policyChooseOne: query.cancel_type ? query.cancel_type : -1,
  ...
  goSearchHotel(e) {
    ...
    //综合筛选
      ...
      const cancel_type = policyChooseOne != -1 ? policyChooseOne : ''
  ...
  scrollDown(e) {
    ...
    query.select('.hotelDetail').boundingClientRect(rect => {
      const {
        scrollTop
      } = e.detail
      const {
      ...
      if (scrollTop > rect.height && !returnTop) {
        this.setData({
          returnTop: true
        })
      } else if (scrollTop < rect.height && returnTop) {
        this.setData({
          returnTop: false
        })
      }
