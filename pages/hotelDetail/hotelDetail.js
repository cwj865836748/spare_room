...
import {formatTime2,formatTime3,getAuth,circleImg,darwRoundRect,textPrewrap} from '../../utils/util.js'
...
Page({
  ...
  onLoad: function (options) {
      //邀请
     const {parentId,isInvite} = options
     console.log(parentId,isInvite)
     parentId&&this.getBindParent(parentId)
     isInvite?this.getLocation().then(location=>{
       const {lat:latitude,lng:longitude} = location
      this.getInit({...options,latitude,longitude})
     }):this.getInit(options)

     this.getTimeInit(options)
  },
  getInit(hotel){
    const {id:hotel_id,longitude,latitude} = hotel
    //酒店详情
    const hotelDetail = request({url:api.hotel.index,data:{hotel_id,longitude,latitude}})
     //酒店设施 酒店轮播图
  ...
  getTimeInit(hotel){
   ...
   const promise2 = [api.hotel.room,api.hotel.near].map(url=>{
    return request({url,data:{hotel_id},header:{starttime,endtime}}) 
  })
  ...
  getBindParent(parent_id){
     ...
     request({url:api.authorization.userBind,data:{parent_id}})
  },
  //是否收藏/取消收藏
  isCollect(){
    const {is_collect,id:hotel_id} =this.data.hotelDetail
    const url =is_collect?api.collection.collectionCancel:api.collection.collectionAdd
     request({url,data:{hotel_id}}).then(res=>{
       if(res.code==200){
         this.setData({
          hotelDetail:{...this.data.hotelDetail,is_collect:!is_collect}
         })
       }
     })
  },
  ...
  closeCalendar(){
   ...
   if((formatTime3(chooseStart)-formatTime3(start_time))||(formatTime3(chooseEnd)-formatTime3(end_time))){
    ...
    this.getTimeInit({id:this.data.hotelId})
  ...
  tabChange(e){
    ...
    const {bedList,hotelList} =  this.data
  ...
  openCloseRoom(e){
     ...
     const {bedList,hotelDetail} =this.data
     bedList[index].isOpen=!bedList[index].isOpen
     App.globalData.bedDetail={...bedList[index],hotelName:hotelDetail.name}
     this.setData({
      bedList
     })
  },
  ...
  //开启地图
  openMap(){
    let {latitude,longitude,address,name} = this.data.hotelDetail
  ...
  goDetail(e){
    ...
    const {id,is_empty} = items
    const {reservation_description} =this.data.hotelDetail
    if(!is_empty){
      return
    }
    wx.navigateTo({
      url: `/pages/roomReservation/roomReservation?id=${id}&reservation_description=${reservation_description}`,
    })
 ...
 shareFriendsSquare(e){
   ...
   if(shareFriendsShow){
    const {slideshowList,hotelDetail} = this.data
    wx.getImageInfo({
      src: slideshowList[0],
      success:  (res)=> {
        this.setData({
...
saveCavans(){
  const {postImg,qrImg,hotelDetail} = this.data
  wx.showToast({
    title: '生成图片中...',
    mask: true,
    icon: 'loading'
 ...
 //酒店地址
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      path:`/pages/hotelDetail/hotelDetail?id=${this.data.hotelId}&parentId=${userId}&isInvite=true`,
