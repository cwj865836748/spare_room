...
Page({
  ...
  data: {
    ...
    isFreeze:false,
    iconList: [{
        icon: '../../images/home_low_price@2x.png',
        name: '全网低价'
      },
      {
        icon: '../../images/home_star_treatment@2x.png',
        name: '五星待遇'
      },
      {
        icon: '../../images/home_housing@2x.png',
        name: '房源保障'
      },
      {
        icon: '../../images/home_professional_services@2x.png',
        name: '专业服务'
      }
    ]
  },
  ...
  getInit() {
    ...
    wx.getStorageSync("token") && getPromise.push(request({
      url: api.index.freeze,
      isHomeUrl: true
    }))
    Promise.all(getPromise).then(res => {
      ...
      this.setData({
        ...
        isFreeze: res[2] && res[2].data
      })
      this.data.isFreeze?wx.hideTabBar():wx.showTabBar()
    })
  },
  ...
  getLocation() {
    ...
    utils.getAuth().then(res => {
      ...
      request({
        ...
        App.globalData.userLocation = defaultCity
        App.getAreaAndRoom()
        this.getStatus()
      })
    })
